...
import com.studyolle.modules.study.form.StudyTxHashForm;
...
public class StudySettingsController {
    ...
    public String studySettingForm(@CurrentAccount Account account, @PathVariable String path, Model model) {
        ...
        model.addAttribute(account);
        model.addAttribute(study);
        model.addAttribute(new StudyTxHashForm());
    ...
    public String closeStudy(@CurrentAccount Account account, @Valid StudyTxHashForm studyTxHashForm,
                             @PathVariable String path, RedirectAttributes attributes) {
        Study study = studyService.getStudyToUpdateStatus(account, path);
        studyService.updateStudyCloseStudyTxHash(study, studyTxHashForm.getCloseStudyTxHash());
    ...
    /* seungho */
    @PostMapping("/study/initDeposit")
    public String updateStudyInitDeposit(@CurrentAccount Account account, @PathVariable String path, Integer newInitDeposit,
                                   Model model, RedirectAttributes attributes) {
        Study study = studyService.getStudyToUpdateStatus(account, path);
        if (!studyService.isValidInitDeposit(newInitDeposit)) {
            model.addAttribute(account);
            model.addAttribute(study);
            model.addAttribute("studyInitDepositError", "스터디 보증금을 다시 입력하세요.");
            return "study/settings/study";
        }

        studyService.updateStudyInitDeposit(study, newInitDeposit);
        attributes.addFlashAttribute("message", "스터디 보증금을 변경했습니다.");
        return "redirect:/study/" + study.getEncodedPath() + "/settings/study";
    }

    @PostMapping("/study/attendPenalty")
    public String updateStudyAttendPenalty(@CurrentAccount Account account, @PathVariable String path, Integer newAttendPenalty,
                                         Model model, RedirectAttributes attributes) {
        Study study = studyService.getStudyToUpdateStatus(account, path);
        if (!studyService.isValidAttendPenalty(newAttendPenalty)) {
            model.addAttribute(account);
            model.addAttribute(study);
            model.addAttribute("studyInitDepositError", "결석 패널티를 다시 입력하세요.");
            return "study/settings/study";
        }

        studyService.updateStudyAttendPenalty(study, newAttendPenalty);
        attributes.addFlashAttribute("message", "결석 패널티를 변경했습니다.");
        return "redirect:/study/" + study.getEncodedPath() + "/settings/study";
    }

    @PostMapping("/study/homeworkPenalty")
    public String updateStudyHomeworkPenalty(@CurrentAccount Account account, @PathVariable String path, Integer newHomeworkPenalty,
                                           Model model, RedirectAttributes attributes) {
        Study study = studyService.getStudyToUpdateStatus(account, path);
        if (!studyService.isValidInitDeposit(newHomeworkPenalty)) {
            model.addAttribute(account);
            model.addAttribute(study);
            model.addAttribute("studyInitDepositError", "과제 패널티를 다시 입력하세요.");
            return "study/settings/study";
        }

        studyService.updateStudyHomeworkPenalty(study, newHomeworkPenalty);
        attributes.addFlashAttribute("message", "과제 패널티를 변경했습니다.");
        return "redirect:/study/" + study.getEncodedPath() + "/settings/study";
    }
    /* seungho */
