...
public class MainController {
    ...
    @GetMapping("/studyAfterLogin")
    public String studyIndex(@CurrentAccount Account account, Model model) {
        if (account != null) {
            Account accountLoaded = accountRepository.findAccountWithTagsAndZonesById(account.getId());
            model.addAttribute(accountLoaded);
            model.addAttribute("everyStudyList",studyRepository.findStudyByClosed(false));
            model.addAttribute("enrollmentList", enrollmentRepository.findByAccountAndAcceptedOrderByEnrolledAtDesc(accountLoaded, true));
            model.addAttribute("studyList", studyRepository.findByAccount(
                    accountLoaded.getTags(),
                    accountLoaded.getZones()));
    ...
    @GetMapping("/eventsAfterLogin")
    public String eventsIndex(@CurrentAccount Account account, Model model) {
        if (account != null) {
            Account accountLoaded = accountRepository.findAccountWithTagsAndZonesById(account.getId());
            model.addAttribute(accountLoaded);
            model.addAttribute("enrollmentList", enrollmentRepository.findByAccountAndAcceptedOrderByEnrolledAtDesc(accountLoaded, true));
    ...
    @GetMapping("/mainAfterLogin")
    public String mainIndex(@CurrentAccount Account account, Model model) {
        if (account != null) {
